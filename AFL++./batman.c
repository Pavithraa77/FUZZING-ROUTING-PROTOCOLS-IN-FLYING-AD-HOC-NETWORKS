...
/* forward declarations of internal statics */
static int isBidirectionalNeigh(
    struct orig_node *orig_node,
    struct orig_node *orig_neigh_node,
    struct bat_packet *in,
    uint32_t recv_time,
    struct batman_if *if_incoming
);

static uint8_t count_real_packets(
    struct bat_packet *in,
    uint32_t neigh,
    struct batman_if *if_incoming
);
...
void fuzz_handle_ogm(unsigned char *buf, int len)
{
    struct bat_packet *bat_packet;
    struct orig_node *orig_node, *orig_neigh_node;
    struct batman_if fake_if;
    uint32_t fake_neigh = 0x0a000001; /* 10.0.0.1 */

    /* semantic flags (local, protocol-only) */
    static int seen_orig  = 0;
    static int seen_neigh = 0;
    static int seen_bidir = 0;

    /* basic size check */
    if (len < (int)sizeof(struct bat_packet))
        return;

    bat_packet = (struct bat_packet *)buf;
    ...
    orig_neigh_node = get_orig_node(fake_neigh);

    /* ---------------- STATE 1: ORIGINATOR SEEN ---------------- */
    if (!seen_orig) {
        /* semantic gate: protocol starts only at seqno 0 */
        if (bat_packet->seqno != 0)
            return;

        seen_orig = 1;
        hit_state(ST_ORIG_SEEN);
    }

    /* ---------------- SEMANTIC UPDATE ---------------- */
    count_real_packets(bat_packet, fake_neigh, &fake_if);

    /* ---------------- STATE 2: NEIGHBOR SEEN ---------------- */
    if (seen_orig && !seen_neigh) {
        seen_neigh = 1;
        hit_state(ST_NEIGH_SEEN);
    }

    /* ---------------- STATE 3: BIDIRECTIONAL LINK ---------------- */
    if (!seen_bidir &&
        ...
        seen_bidir = 1;
        hit_state(ST_BIDIRECTIONAL);
    }

    /* ---------------- STATE 4: ROUTING UPDATE ---------------- */
    if (seen_bidir) {
...
{    
#ifdef AFL_FUZZING
      if (gw_list.next == NULL || gw_list.prev == NULL)
         return;
#endif

       printf("Address of update_gw_list: %p\n", (void *)&update_gw_list);
    printf("Address of gw_list: %p\n", (void *)&gw_list);
	...
	list_for_each_safe( gw_pos, gw_pos_tmp, &gw_list ) {
         printf("gw_pos: %p, gw_pos_tmp: %p\n", (void *)gw_pos, (void *)gw_pos_tmp);

		gw_node = list_entry(gw_pos, struct gw_node, list);
          printf("gw_node: %p, gw_node->orig_node: %p\n", (void *)gw_node, (void *)gw_node->orig_node);
